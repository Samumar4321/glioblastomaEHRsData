...
test_that("descriptiveTableUtrecht2019dataset(savePath = 'tables/') return a table1 object and save a png file", {
...
test_that("descriptiveTableUtrecht2019dataset(savePath = 'tables/utrecht_test_table.html') return a table1 object and save a html file", {
  path <- file.path(tempdir(), "tables/utrecht_test_table.html")
...
test_that("descriptiveTableUtrecht2019dataset(savePath = 'imgs') return a table1 object and generates a 'Directory not found' warning'", {
...
test_that("descriptiveTableUtrecht2019dataset(savePath = 'utrecht_test_table.jpeg') return a table1 object and generates a 'Extension not supported' warning", {
  path <- file.path(tempdir(), "utrecht_test_table.jpeg")
  expect_warning(t1 <- descriptiveTableUtrecht2019dataset(savePath = path))
  expect_s3_class(t1, c("table1", "character"))
})
