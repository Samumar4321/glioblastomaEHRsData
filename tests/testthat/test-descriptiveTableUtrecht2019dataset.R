...
test_that("descriptiveTableUtrecht2019dataset(savePath = 'images/utrecht_test_table.png') return a table1 object and save a png file", {
...
test_that("descriptiveTableUtrecht2019dataset(savePath = 'utrecht_test_table.html') return a table1 object and save a html file", {
  path <- testthat::test_path("", "utrecht_test_table.html")
  t1 <- descriptiveTableUtrecht2019dataset(savePath = path)
  expect_s3_class(t1, c("table1", "character"))
  expect_true(file.exists(path))
})
