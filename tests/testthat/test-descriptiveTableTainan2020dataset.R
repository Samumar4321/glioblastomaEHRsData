...
test_that("descriptiveTableTainan2020dataset(savePath = 'images/tainan_test_table.png') return a table1 object and save a png file", {
...
test_that("descriptiveTableTainan2020dataset(savePath = 'tainan_test_table.html') return a table1 object and save a html file", {
  path <- testthat::test_path("", "tainan_test_table.html")
  t1 <- descriptiveTableTainan2020dataset(savePath = path)
  expect_s3_class(t1, c("table1", "character"))
  expect_true(file.exists(path))
})
