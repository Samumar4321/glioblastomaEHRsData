library(pacman)
p_load(table1, readxl, tidyverse)
...
#tabella uguale con un custom render
table1(strata, labels, groupspan=c(3, 3, 1), render.continuous = c("Media (SD)" = "Mean (SD)"), topclass = "Rtable1-shade")

#render custom per singole variabili
rndr <- function(x, name, ...) {
  if (!is.numeric(x)) return(render.categorical.default(x))
  what <- switch(name,
                 age_years = "Median [Min, Max]",
                 OS_months  = "Mean (SD)")
  what <- switch(name,
                 age_years = 'names<-'(what, "Mediana [Max, Min]"),
                 OS_months = 'names<-'(what, "Media (DS)"))
